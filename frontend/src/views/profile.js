...
export const Profile = (props) => {
  ...
  const editProject = async (id) =>{
    const res = await fetch(`http://127.0.0.1:5000/api/project/${id}`)
      const data = await res.json();
      console.log(data)
                    ...
                    {lista.map((item, i) => (
                      <div className="col-12 col-md-6 col-lg-4 mb-5">
                        <div className="card">
                          ...
                          <div className="card-body">
                            <button className="btn btn-success me-1" onClick={() => editProject(item.id)}>
                              <i className="bi bi-pencil-square"></i> Edit
                            </button>
                            <button className="btn btn-danger me-1" onClick={()=> deleteUser(item.id)}>
